...
def get_condition(prj_name):
    ...
    pos, trap_energy = float(splits[0][1:]), float(splits[1][1:])
    return pos, trap_energy


def read_current(prj_path):
    current_folder = os.path.join(prj_path, 'Current')
    file_path = comm.searchFilePathByTime(current_folder, 'eTAT', 1e-8)
...
def main():
    main_prj = os.path.join(Directory_CPB2015, 'trap_energy_position')
    ...
    comm.write_data(os.path.join(main_prj, 'pos_energy.txt'), pos_list, trap_energy_list, curr_dens_list)
