...
def main():
    ...
    trap_energy_list, pos_list, vfb_shift_list = [], [], []
    for prj_name in os.listdir(main_prj):
        if prj_name == 'sample':
            continue
        if 'txt' in prj_name:
            continue
        trap_energy, pos = get_condition(prj_name)
    ...
    comm.write_data(os.path.join(main_prj, 'energy_pos.txt'), pos_list, trap_energy_list,
                    vfb_shift_list)
    return None
