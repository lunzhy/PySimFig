...
for prj_index, prj_name in enumerate(reversed(Prj_list)):
  ...
  for time_index, time_str in enumerate(reversed(Time_list)):
    ...
    file = common.searchFileNameByTime(trap_path, Trap_file_pattern, time)
    yCoords, eTrapped = common.getDataAlongY_1D(file, 4) # the third column in the file
    leg_label = 'Vprg = %s@t = %0.es' % (prj_name, time)
    ax.plot(yCoords, eTrapped, lw=4, ls=common.getLinestyle(ls_index), color=common.getColor(color_ind), label=leg_label)
